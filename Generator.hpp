...
struct Generator {
  ...
  public:
    Generator() {}
    Generator(const unsigned&, const double&, const double&, const int&);
...
Generator::Generator(const unsigned& t_size,
...
const std::vector<double> Generator::dtheta_dt(const std::vector<double>& t_v_list) const {
    /*
        d theta / dt = v
...
const std::vector<double> Generator::dv_dt(const std::vector<double>& t_theta_list,
...
void Generator::RK4_step(const double& t_dt) {
    using namespace linearAlgebra;
...
void Generator::run(const unsigned& t_max_iteration, const double& t_dt = 0.005) {
    //* Save delta t
    dt = t_dt;
...
void Generator::save(const std::string& t_file_path = "") const {
    /*
